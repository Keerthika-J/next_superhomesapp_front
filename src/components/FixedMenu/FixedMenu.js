...
function FixedMenu() {
  ...
  const [className, setZoom] = useState('');
  ...
  return (
    <nav className="header-main">
      ...
      <div className='mobile_header w-full flex items-center justify-between' style={{ display: show ? "flex" : "none" }}>
        <div className="w-12 h-12">
          <Link href="/"><img className="cursor-pointer" src="/images/logo-white.png" alt="Logo" ></img></Link>
        </div>
        <div className="">
          <button className="menu-button rounded-full" onClick={toggleDrawer}><img src="/images/hamburger-icon.png" alt="Hamburger icon" className="w-8 h-6 invert"></img></button>
        </div>
      </div>
    </nav>
  );
